...
mkdir -p "${PREFIX}"/bin/platformio/base-node-rpc-freertos/default
...
cp -ra "${SRC_DIR}"/lib/BaseNodeRpcFreeRtos "${PREFIX}"/include/Arduino/BaseNodeRpcFreeRtos
# Copy compiled firmware to Conda bin directory
cp -a "${SRC_DIR}"/platformio.ini "${PREFIX}"/bin/base-node-rpc-freertos
cp -a "${SRC_DIR}"/.pioenvs/default/firmware.hex "${PREFIX}"/bin/base-node-rpc-freertos/default/firmware.hex
rc=$?; if [[ $rc != 0  ]]; then exit $rc; fi
