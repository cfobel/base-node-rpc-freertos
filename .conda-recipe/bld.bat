...
md "%PREFIX%"\Library\bin\platformio\base-node-rpc-freertos\default
endlocal
...
xcopy /S /Y /I /Q "%SRC_DIR%"\lib\BaseNodeRpcFreeRtos "%PREFIX%"\Library\include\Arduino\BaseNodeRpcFreeRtos
REM Copy compiled firmware to Conda bin directory
copy "%SRC_DIR%"\platformio.ini "%PREFIX%"\Library\bin\platformio\base-node-rpc-freertos
copy "%SRC_DIR%"\.pioenvs\default\firmware.hex "%PREFIX%"\Library\bin\platformio\base-node-rpc-freertos\default\firmware.hex
if errorlevel 1 exit 1
